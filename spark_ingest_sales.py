...
df = spark.read.option("header", True).csv("C:/Users/Nikolaus/Desktop/fp_midk/Superstore.csv")

# (Opsional) Bisa tambahkan transformasi di sini misalnya cleaning
# df = df.withColumn(...)

# Langkah 2: Simpan langsung ke HDFS
df.write.mode("overwrite").option("header", True).csv("hdfs://localhost:9000/user/data/Superstore.csv")
